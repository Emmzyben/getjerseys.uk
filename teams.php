...
if ($teamId) {
    ...
    $stmt = $conn->prepare("SELECT teams.name AS team_name, categories.name AS category_name, categories.type FROM teams JOIN categories ON teams.category_id = categories.id WHERE teams.id = :teamId");
    $stmt->execute(['teamId' => $teamId]);
    $team = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($team) {
        $title = htmlspecialchars("{$team['team_name']} Jerseys");
        $subtitle = "Explore jerseys for {$team['team_name']} ({$team['category_name']})";

        $stmt = $conn->prepare("SELECT jerseys.*, teams.name AS team_name, categories.name AS category_name 
                                FROM jerseys 
                                JOIN teams ON jerseys.team_id = teams.id 
                                JOIN categories ON teams.category_id = categories.id 
